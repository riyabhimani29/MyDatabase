...
/****** Object:  StoredProcedure [dbo].[QR_Mst_GetDetail]    Script Date: 26-04-2026 19:38:19 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[QR_Mst_GetDetail] @QR_Id INT = 6028,          
                                              @Typ         VARCHAR(10) = 'OUT'          
AS          
    SET nocount ON          
    ...
    if ( @Typ = 'GRN' )          
      ...
      CONVERT(NUMERIC(18, 0), PO_MST.IGSTTotal)   AS igsttotal,          
         ...
         PO_MST.PO_Date,          
                 ...
                 PO_DTL.orderqty,          
                 /*ISNULL(PO_DTL.PendingQty,0)*/ Tot.reccnt          AS          
                 ReceiveQty,          
    ...
    CONVERT(VARCHAR, PO_DTL.item_id) + ',' + CONVERT(VARCHAR, PO_DTL.po_id)--,                          
      ...
      begin          
         ...
         Isnull(PO_DTL.PendingQty, 0) ELSE 0 END) AS PendingQty          
          ...
          WHERE  PO_MST.PO_Id IN (SELECT TOP 1 PO_Id          
                                  FROM   GlassQR_Dtl WITH (nolock)          
                                  WHERE  GlassQR_Dtl.PO_Id = @QR_Id  and GlassQR_Dtl.QR_Typedtl ='OUT'        
                                  GROUP  BY PO_Id          
                                  HAVING Count(*) >= 1          
                                  ORDER  BY PO_Id DESC)          
          ORDER  BY PO_MST.PO_Id DESC          
