...
/****** Object:  StoredProcedure [dbo].[PO_MST_Get_Pending]    Script Date: 26-04-2026 19:23:43 ******/
SET ANSI_NULLS ON
GO
...
ALTER  PROCEDURE [dbo].[PO_MST_Get_Pending] @PO_Id       INT = 0,                
                                    @Dept_ID int = 0,                  
                                    @Supplier_Id INT =0,                                        
                                    @PO_Type     VARCHAR(1)=''  ,          
...
AS                                        
    ...
    when Order_Type = 'PO-RF' then 'Reinforcement PO'
    when Order_Type = 'PO-SH' then 'Sheet PO'
    when Order_Type = 'PO-ALU' then 'Aluminium PO'
     else         
     Order_Type        
     end        
     AS Order_Type2,                                         
           ...
           CASE                                        
             ...
             ELSE ( CASE WHEN PR_MST.PO_Type = 'D' THEN 'Draft'                                        
                      ELSE ( CASE WHEN PR_MST.PO_Type = 'C' THEN 'Cancel'                                
          WHEN PR_MST.PO_Type = 'Q' THEN 'Force Close'                               
          WHEN PR_MST.PO_Type = 'X' THEN 'Delete'  
          WHEN PR_MST.PO_Type = 'P' THEN 'Pending Approval'
 ...
 FROM   PR_MST WITH (nolock)                                        
           ...
           OUTER apply (SELECT Sum(Isnull(PR_DTL.orderqty, 0))   AS OrderQty,                                 
                        ...
                        WHERE  PR_DTL.PR_Id = PR_MST.PR_Id) AS Tbl                                        
   ...
   and  PR_MST.Dept_ID  = case when   @Dept_ID = 0 then PR_MST.Dept_ID else @Dept_ID end                  
         ...
         --  AND PO_MST.supplier_id = CASE                                        
          ...
          --  END 
            AND (
        (PR_MST.PO_Type = 'P'  AND PR_MST.AuthorizedBy_Id = @User)
     OR (PR_MST.PO_Type = 'PP' AND PR_MST.CheckedBy_Id    = @User)
      )
   ...
   LEFT JOIN M_SupplierDtl WITH (nolock) ON PR_DTL.SupDetail_Id = M_SupplierDtl.SupDetail_Id                                        
