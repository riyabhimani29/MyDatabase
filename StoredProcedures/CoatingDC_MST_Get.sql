...
/****** Object:  StoredProcedure [dbo].[CoatingDC_MST_Get]    Script Date: 26-04-2026 17:48:14 ******/
SET ANSI_NULLS ON
GO
...
ALTER  PROCEDURE [dbo].[CoatingDC_MST_Get]                                                 
    @DC_Id INT = 0 ,                                                
...
AS                                                                    
 ...
 SELECT dc_mst.Dept_ID, 
       ...
       ( CASE WHEN dc_mst.codc_type = 'D' THEN                   
          CONVERT (NUMERIC(18, 3), ( CASE WHEN Isnull(dc_mst.igst, 0) = 0 THEN                  
                                            CONVERT ( NUMERIC(18, 3), ( Isnull(tbl.coating_value, 0) + ( Isnull(tbl.coating_value, 0) * Isnull(tbl_cgst.master_numvals, 0) ) + (                  
         ...
         END ))                  
           ELSE CONVERT(NUMERIC(18, 3), dc_mst.netamount) END ) AS netamount,                  
       dc_mst.remark,                  
       --CASE                  
...
FROM   dc_mst WITH (nolock)   
       LEFT JOIN M_Department WITH (nolock) ON M_Department.Dept_ID = dc_mst.Dept_Id
...
/****************************** Details Query ****************************************/                  
  ...
  --Row_number()   OVER(  ORDER BY dc_dtl.dc_id)                AS SrNo,                                                             
      ...
      dc_dtl.running_feet,                                                          
      dc_dtl.rate_feet,                                                          
      CONVERT(NUMERIC(18, 3), dc_dtl.coating_value)  AS coating_value,                                      
      dc_dtl.total_weight,                                                          
      CONVERT(NUMERIC(18, 3), dc_dtl.material_value) AS material_value,                                                          
      dc_dtl.scrap_qty,                                                          
      dc_dtl.scrap_length  ,                            
      DC_Dtl.DC_Width   ,                                                        
      DC_Dtl.Scrap_Width ,      
